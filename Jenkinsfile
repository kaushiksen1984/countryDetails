...
def Deploy_Application(run) {
	stage('Deploy_Application') {
		if(run) {
			deploy adapters: [tomcat9(credentialsId: 'TomcatDepoymentId', path: '', url: 'http://localhost:8085')], contextPath: '/v1', war: 'target/countryDetails.war'
		}
	}
}
...
def Run_BDD(run) {
	stage('Run_BDD') {
		if(run) {
			sh "mvn test -PrunBDD -Dtest=CountryDetailsBDDRunner -Dkarate.baseURL=http://localhost:8085/v1/country"
